...
import { getScanReport } from "../services/reportService.js";
...
export const getScanReportHandler = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id;
  if (!id) throw new HttpError(400, "scan id is required");
  const report = await getScanReport(id);
  if (!report) throw new HttpError(404, "Scan not found");
  res.json(report);
});
